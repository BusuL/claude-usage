...
// Pricing per 1M tokens (Claude API rates)
// Cache-read tokens are ~10x cheaper than regular input tokens
const PRICING = {
...
function estimateCost(model, pureInputTokens, outputTokens, cacheWriteTokens, cacheReadTokens) {
  const p = getModelPricing(model);
  return (
    (pureInputTokens   || 0) * p.input      +
    (outputTokens      || 0) * p.output     +
    (cacheWriteTokens  || 0) * p.cacheWrite +
...
function extractSessionData(entries) {
  ...
  for (const entry of entries) {
    if (entry.type === 'user' && entry.message?.role === 'user') {
      ...
      // Truncate prompt text to 500 chars to keep payload manageable
    ...
    if (entry.type === 'assistant' && entry.message?.usage) {
      ...
      const pureInputTokens  = usage.input_tokens || 0;
      const cacheWriteTokens = usage.cache_creation_input_tokens || 0;
      const cacheReadTokens  = usage.cache_read_input_tokens || 0;
      const inputTokens      = pureInputTokens + cacheWriteTokens + cacheReadTokens;
...
async function parseAllSessions() {
  ...
  for (const projectDir of projectDirs) {
    ...
    // Collect .jsonl files from direct level and one level deeper (UUID subfolders)
    const filePaths = [];
      ...
      if (entry.endsWith('.jsonl')) {
        ...
        try {
          if (fs.statSync(entryPath).isDirectory()) {
            for (const subFile of fs.readdirSync(entryPath).filter(f => f.endsWith('.jsonl'))) {
    ...
    for (const filePath of filePaths) {
      ...
      // Calculate totals from ALL queries before capping the stored array
      let inputTokens = 0, outputTokens = 0;
      let pureInputTokens = 0, cacheWriteTokens = 0, cacheReadTokens = 0;
      for (const q of queries) {
        inputTokens      += q.inputTokens;
        outputTokens     += q.outputTokens;
        pureInputTokens  += q.pureInputTokens  || 0;
      ...
      sessions.push({
        ...
        // Cap to 300 entries for drill-down to keep JSON payload manageable
        queries: queries.slice(0, 300),
        inputTokens,
        outputTokens,
        totalTokens,
        pureInputTokens,
        cacheWriteTokens,
        cacheReadTokens,
        estimatedCost: estimateCost(primaryModel, pureInputTokens, outputTokens, cacheWriteTokens, cacheReadTokens),
      });
  ...
  const grandTotals = {
    ...
    totalEstimatedCost: sessions.reduce((sum, s) => sum + s.estimatedCost, 0),
    avgTokensPerQuery: 0,
